        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    self.root = node
                    self.fileURL = URL(fileURLWithPath: self.targetPath)
                    self.originalBytes = Int64(data.count)
                    self.editedBytes = Int64(data.count)
                    self.message = "System disable.plist loaded."
